...
def get_dfs(d):
    ...
    for folder in os.listdir(d):
        '''
        if '.zip' in f"{d}/{folder}":
            with zipfile.ZipFile(f"{d}/{folder}", 'r') as zip_ref:
                zip_ref.extractall(f"{d}/{folder}".replace('.zip', ''))
        '''
        try:
...
def get_lon_lat(df, nombre):
    ...
    for la, lo in zip(lat, lon):
        ...
        latnew.append(a)
        lonnew.append(o)
    return {nombre: {'lon':lon, 'lat':lat}}


def get_clusters(nombre):
    lon, lat = mydic[nombre]['lon'], mydic[nombre]['lat']
    scaled_lon = scaler_lon.transform(lon)
    scaled_lat = scaler_lat.transform(lat)
    clusters = kmeans.transform(pd.DataFrame({'x': scaled_lat, 'y':scaled_lon}))
    #newdic={'clusters':clusters}
    return clusters


def get_suma_var(nombre):
    #name = f"suma_{nombre}".replace(' ', '_')
    contador = dict(Counter(mydic[nombre][cluster]))
...
def get_individual_df(nombre):
    clusters = []
    contadores = []
    for k, v in mydic[nombre]['clusters'].items():
        clusters.append(k)
        contadores.append(v)
...
if __name__ == '__main__':
    ...
    print('####### MERGEANDO #########')
    df_final = reduce(lambda left,right: pd.merge(left,right,on='cluster'), processed_dfs)
    df_final.to_csv('df_datos_nomecalles.csv', header=True, index=False)
    print('********** Finalizado ***********')
